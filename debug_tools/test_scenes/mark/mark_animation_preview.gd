...
func _transition_to(path: NodePath) -> void:
	...
	if _state_machine.state == null:
		...
		return
	_state_machine.transition_to(path)
...
func _trigger_knockout() -> void:
	...
	_state_machine.transition_to(STEP_KO, {"launch_vector": KO_LAUNCH_VECTOR.normalized()})
...
func _render_help() -> void:
	...
	if _sprite != null:
		...
		frame = _sprite.frame
	_help.text = "SPACE start/stop cycle | I idle | W walk | H hurt | 1/2/3 combos | J jump | A air attack | K knockout | T 1/4 speed\nState: %s | Tree: %s | Anim: %s #%d | TimeScale: %.2f" % [_last_state, tree_active, anim_name, frame, speed]
